...
> 本文档记录对 `YsnSkin-V0.1.50` 发布包的逆向分析结论，是本项目的设计依据。
...
```
┌─ 选择层 ──────────────────────────────────────────────────────┐
│ YsnSkin：WPF 悬浮窗（模式一）/ Pengu 插件改客户端 DOM（模式二）│
│ 本项目：tkinter 悬浮窗（模式一）                               │
└──────────────┬────────────────────────────────────────────────┘
               │ LCU HTTPS（lockfile 凭据）
┌──────────────▼────────────────────────────────────────────────┐
│ 编排层：YsnSkin = C# WPF 客户端（YsnSkin.dll）                 │
│   · WebSocket 桥 ws://127.0.0.1:50123/ysnskin-bridge/         │
│   · 皮肤切换状态机 Queued→Building→Applying→Ready             │
│   · 授权：RSA 票据 + 设备绑定 + 引擎代数（商业护城河）         │
│ 本项目：patcher.py 驱动 + 简单状态机（无授权）                 │
└──────────────┬────────────────────────────────────────────────┘
               │ 子进程 + 管道
┌──────────────▼────────────────────────────────────────────────┐
│ 构建层：YsnSkin = rift-overlay.exe（50MB Rust 引擎，闭源）     │
│   · skin-overlay GAME_DIR CHAMPION SKIN_NUM --out OVERLAY_DIR  │
│   · 皮肤图改写（entry alias skinN→skin0）+ delta 增量          │
│ 本项目：wad.py + prop.py + overlay.py（自写 Python 实现）      │
└──────────────┬────────────────────────────────────────────────┘
               │ 命令行 + 命名管道
┌──────────────▼────────────────────────────────────────────────┐
│ 注入层：ltk_patcher_host.exe（宿主）+ ltk_patcher_dll.dll     │
│   · SetWindowsHookEx 注入游戏进程                              │
│   · CreateFileA IAT 钩子 → WAD 重定向                          │
│   · CRYPTO_free 钩子 → 绕过签名校验                            │
│   · \\.\pipe\ltk-patcher-pipe 管道通信                         │
